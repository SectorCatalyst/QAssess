...
async function run(): Promise<void> {
  const databaseUrl = process.env.MIGRATION_DRY_RUN_DATABASE_URL ?? requiredEnv('DATABASE_URL');
  ...
  try {
    ...
    for (const file of files) {
      ...
      process.stdout.write(`Dry-run applying ${file.version}...\n`);
      ...
      applied += 1;
      process.stdout.write(`Dry-run applied ${file.version}.\n`);
    }

    process.stdout.write(`Dry-run migration complete. applied=${applied} skipped=${skipped}\n`);
  } finally {
    await pool.end();
  }
}
...
run().catch((error: unknown) => {
  process.stderr.write(`Migration dry-run failed: ${String(error)}\n`);
  process.exitCode = 1;
});
